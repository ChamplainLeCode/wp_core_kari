  ...
  void screen(String name, RouteMode mode, {dynamic parameter, RouteDirection direction = RouteDirection.LEFT_TO_RIGHT, cupertino.BuildContext context}){
    ...
    switch(mode){
      ...
      case RouteMode.EMPTY:
        cupertino.Navigator.of(ctx).pushNamedAndRemoveUntil(name, (_)=>false, arguments: parameter);
        break;
      default: cupertino.Navigator.of(ctx).pop(false);
    }
  }
...
class Router {
  ...
  static cupertino.Widget initialScreen(){
    return screens.firstWhere((routeItem) => routeItem[#initial]??false)[#screen];
  }

  static cupertino.Widget componentForRouteName(String s) => screens.firstWhere((routeItem) => routeItem[#name] == s )['screen'];
